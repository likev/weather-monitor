...
monitorElement.visibility = function(type, content){
	...
	stationList.each((index, item)=>{
		...
		name = tdList.eq(2).text(),
		//TmaxArray = tdList.eq(5).text().split(' '),
		value = tdList.eq(12).text().trim(),
		occurTime = tdList.eq(13).text().trim();
...
monitorElement.Tmax4New = function(type, content){
	const {thisDownTimeStr, stationList, $} = getInfo(type, content);
	...
	stationList.each((index, item)=>{
		...
		name = tdList.eq(2).text(),
		//TmaxArray = tdList.eq(5).text().split(' '),
		value = tdList.eq(6).text().trim(),
		occurTime = tdList.eq(7).text().trim().slice(-16);
...
monitorElement.Tmax4Old = function(type, content){
	const {thisDownTimeStr, stationList, $} = getInfo(type, content);
	...
	stationList.each((index, item)=>{
		...
		//TmaxArray = tdList.eq(5).text().split(' '),
		value = tdList.eq(7).text().trim(),
		occurTime = tdList.eq(8).text().trim();
...
monitorElement.hourRain4New = function(type, content){
	const {thisDownTimeStr, stationList, $} = getInfo(type, content);
...
monitorElement.hourRain4Old = function(type, content){
	const {thisDownTimeStr, stationList, $} = getInfo(type, content);
...
monitorElement.windMax = function(type, content){
	...
	stationList.each((index, item)=>{
		...
		windArray = tdList.eq(18).text().split(' '),
		value = windArray[1],
...
monitorElement.windMaxPeak = function(type, content){
	...
	stationList.each((index, item)=>{
		...
		windArray = tdList.eq(19).text().split(' '),
		value = windArray[1],
		direction = windArray[0],
		occurTime = windArray[2];
...
monitorElement.windMax4New = function(type, content){
	const {thisDownTimeStr, stationList, $} = getInfo(type, content);
	...
	stationList.each((index, item)=>{
		...
		name = tdList.eq(2).text(),
		//TmaxArray = tdList.eq(5).text().split(' '),
		value = tdList.eq(16).text().trim(),
		direction = tdList.eq(17).text().trim(),
		occurTime = tdList.eq(18).text().trim().slice(-16);
...
monitorElement.windMaxPeak4New = function(type, content){
	const {thisDownTimeStr, stationList, $} = getInfo(type, content);
	...
	stationList.each((index, item)=>{
		...
		value = tdList.eq(19).text().trim(),
		direction = tdList.eq(20).text().trim(),
		occurTime = tdList.eq(21).text().trim().slice(-16);
...
monitorElement.windMax4Old = function(type, content){
	const {thisDownTimeStr, stationList, $} = getInfo(type, content);
	...
	stationList.each((index, item)=>{
		...
		value = tdList.eq(19).text().trim(),
		direction = tdList.eq(20).text().trim(),
		occurTime = tdList.eq(21).text().trim();
...
monitorElement.windMaxPeak4Old = function(type, content){
	const {thisDownTimeStr, stationList, $} = getInfo(type, content);
	...
	stationList.each((index, item)=>{
		...
		value = tdList.eq(22).text().trim(),
		direction = tdList.eq(23).text().trim(),
		occurTime = tdList.eq(24).text().trim();
...
monitorElement.Tmax6 = monitorElement.Tmax4New;
monitorElement.hourRain1 = monitorElement.hourRain4New;
monitorElement.hourRain6 = monitorElement.hourRain4New;
monitorElement.windMax6 = monitorElement.windMax4New;
monitorElement.windMaxPeak6 = monitorElement.windMaxPeak4New;

let loop = function( {ysType='lyBasic',city='ly', citydown='all', sOccTime5='',ys='2',xiansa='1', elements=[]}={} ){
	var postData = {
	  city,
	  citydown,
	  sOccTime5,
...
exports.start = function(){
	...
	loop({
			ysType: 'lyBasic', 
			ys: '2', 
			elements:['Tmax','hourRain','windMax','windMaxPeak']
		});
	...
	loop({
			ysType: 'lyOld4', 
			ys: '4', 
			elements:['Tmax4Old','hourRain4Old','windMax4Old', 'windMaxPeak4Old']
		});
	
	loop({
			ysType: 'lyNew4', 
			ys: '41', 
			elements:['Tmax4New','hourRain4New','windMax4New', 'windMaxPeak4New']
		});
